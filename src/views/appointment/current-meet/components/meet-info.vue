  ...
  methods: {
    ...
    getDateilsInfo(id) {
      ...
      getMeetingDetailApi({ id: id }).then(res => {
        ...
        if (this.ruleForm.category == 2) {
          ...
          // 预约类型
          this.categoryStr = `${this.category[this.ruleForm.category]}（${
            this.repetitionType[this.ruleForm.repetition_type]
          }）`;
        }
        if (this.dataType == 1) {
          this.ruleForm.meeting_type_id = this.ruleForm.is_secret
            ? "*"
            : this.ruleForm.meeting_type_name;
        } else {
          this.ruleForm.meeting_type_id = this.ruleForm.is_secret
            ? "*"
            : this.ruleForm.meeting_type_id;
        }
        // 茶点服务
    ...
    repeatEndTimeHandle() {
      ...
      this.reapSessions = dec
        ? ` ${this.$t("public.total")}${dec}${this.$t("public.meetings")}`
        : "";
    ...
    editOtherParticipant(index) {
      if (!isNaN(index)) {
        ...
        this.outParticipantGuids.unshift({
          name: "",
          email: "",
          phone: "",
          nameError: false,
